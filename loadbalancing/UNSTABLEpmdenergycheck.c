...
----------------------------------------------------------------------*/
  ...
  for (nZ=0; nZ<InitUcell[2]; nZ++) {
    ...
    if (sid == 1) {
        ...
        for (j = 0; j < original_n * 0.15; j++) {  // Add 60% more
            for (a = 0; a < 3; a++) {
...
void half_kick() {
    int i,a;
    double vmax = 100.0;  // Maximum allowed velocity
    ...
    for (i=0; i<n; i++) {
        for (a=0; a<3; a++) {
            ...
            if (fabs(rv[i][a]) > vmax) {
                printf("Process %d: WARNING - Velocity capped for atom %d\n", sid);
                rv[i][a] = (rv[i][a] > 0) ? vmax : -vmax;
            }
        }
    }
}
...
void dynamic_load_balance(int step) {
    ...
    // Gather atom counts
    int* local_atom_counts = (int*)malloc(total_procs * sizeof(int));
    MPI_Allgather(&n, 1, MPI_INT, local_atom_counts, 1, MPI_INT, MPI_COMM_WORLD);
    
    // Calculate imbalance
    int max_atoms = local_atom_counts[0];
    int min_atoms = local_atom_counts[0];
    ...
    double imbalance = (double)max_atoms / min_atoms;
    
    if (sid == 0) {
        printf("\nLoad balance check at step %d:\n", step);
        printf("Imbalance ratio: %.3f (threshold: %.3f)\n", imbalance, load_imbalance_threshold);
    }
    ...
    if (imbalance > load_imbalance_threshold) {
        ...
        // For each process with excess atoms
        if (n > target_atoms + 100) {  // Having excess atoms
            int excess = n - target_atoms;
            
            // Find neighbor with fewer atoms
            int neighbor = (sid == total_procs - 1) ? sid - 1 : sid + 1;
            if (local_atom_counts[neighbor] < target_atoms) {
                int transfer_count = min(excess/2, target_atoms - local_atom_counts[neighbor]);
                transfer_count = min(transfer_count, 100);  // Limit transfer size
                
                // Find atoms near boundary
                int* atoms_to_send = (int*)malloc(transfer_count * sizeof(int));
                int found = 0;
                
                // For upper neighbor, send atoms with largest z
                // For lower neighbor, send atoms with smallest z
                if (neighbor > sid) {
                    // Find atoms with largest z coordinates
                    for (int i = 0; i < n && found < transfer_count; i++) {
                        if (r[i][2] > al[2]/2.0) {
                            atoms_to_send[found++] = i;
                        }
                    }
                } else {
                    // Find atoms with smallest z coordinates
                    for (int i = 0; i < n && found < transfer_count; i++) {
                        if (r[i][2] < al[2]/2.0) {
                            atoms_to_send[found++] = i;
                        }
                ...
                if (found > 0) {
                    printf("Process %d: Sending %d atoms to process %d\n",
                           sid, found, neighbor);
                    
                    // Pack transfer buffer
                    double* send_buffer = (double*)malloc(found * 6 * sizeof(double));
                    for (int i = 0; i < found; i++) {
                        int atom_idx = atoms_to_send[i];
                        for (int a = 0; a < 3; a++) {
                            send_buffer[i*6 + a] = r[atom_idx][a];
                            send_buffer[i*6 + 3 + a] = rv[atom_idx][a];
                        }
                    }
                    
                    // Exchange atom counts
                    MPI_Send(&found, 1, MPI_INT, neighbor, 0, MPI_COMM_WORLD);
                    MPI_Send(send_buffer, found * 6, MPI_DOUBLE, neighbor, 1, MPI_COMM_WORLD);
                    
                    // Remove sent atoms (by swapping with last atoms)
                    for (int i = 0; i < found; i++) {
                        int idx = atoms_to_send[i];
                        if (idx < n-1) {
                            for (int a = 0; a < 3; a++) {
                                r[idx][a] = r[n-1][a];
                                rv[idx][a] = rv[n-1][a];
        ...
        // Receive atoms if we have fewer than target
        if (n < target_atoms) {
            int incoming_count;
            MPI_Status status;
            
            // Check for incoming atoms from neighbors
            MPI_Recv(&incoming_count, 1, MPI_INT, MPI_ANY_SOURCE, 0,
                    MPI_COMM_WORLD, &status);
            
            if (incoming_count > 0) {
                double* recv_buffer = (double*)malloc(incoming_count * 6 * sizeof(double));
                MPI_Recv(recv_buffer, incoming_count * 6, MPI_DOUBLE,
                        status.MPI_SOURCE, 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                
                // Unpack received atoms
                for (int i = 0; i < incoming_count; i++) {
                    for (int a = 0; a < 3; a++) {
                        r[n][a] = recv_buffer[i*6 + a];
                        rv[n][a] = recv_buffer[i*6 + 3 + a];
                    }
                    n++;
                }
                
                free(recv_buffer);
                printf("Process %d: Received %d atoms from process %d\n",
                       sid, incoming_count, status.MPI_SOURCE);
            }
        }
    }
    ...
    // Update neighbor lists after load balancing
    atom_copy();
}
...
//        int max_transfer = 100;  // Only transfer up to atoms at a time
...
void compute_accel() {
    ...
    double rMin2 = 0.1;  // Minimum allowed separation squared
    double maxForce = 1000.0;  // Maximum allowed force
    ...
    // Build linked cell lists
    for (i=0; i<n+nb; i++) {
        // Enforce periodic boundaries first
        for (a=0; a<3; a++) {
            while (r[i][a] >= al[a]) r[i][a] -= al[a];
            while (r[i][a] < 0.0) r[i][a] += al[a];
        }

        // Calculate cell indices with safety checks
        for (a=0; a<3; a++) {
            mc[a] = (int)(r[i][a]/rc[a]) + 1;  // +1 for padding cell
            if (mc[a] < 0 || mc[a] >= lc2[a]) {
                printf("Process %d: Warning - Adjusting cell index for atom %d\n", sid, i);
                mc[a] = (mc[a] < 0) ? 0 : (lc2[a] - 1);
            }
        }
                ...
                while (j != EMPTY) {
                    if (j != i) {
                        ...
                        if (i<j && rr<rrCut) {
                            ...
                            if (rr < rMin2) {
                                printf("Process %d: WARNING - Atoms %d and %d too close (r=%.6f)\n, will be adjusted",
                                       sid, i, j, sqrt(rr));
                                // Adjust positions to minimum separation
                                double scale = sqrt(rMin2/rr);
                                for (a=0; a<3; a++) {
                                    double dr_half = dr[a] * (scale - 1.0) * 0.5;
                                    r[i][a] += dr_half;
                                    r[j][a] -= dr_half;
                                }
                                rr = rMin2;
                            ...
                            // Calculate forces with magnitude cap
                            ri2 = 1.0/rr;
                            ri6 = ri2*ri2*ri2;
                            r1 = sqrt(rr);
                            fcVal = 48.0*ri2*ri6*(ri6-0.5) + Duc/r1;
                            
                            if (fabs(fcVal) > maxForce) {
                                fcVal = (fcVal > 0) ? maxForce : -maxForce;
                            }
    ...
    // Sum potential energy across processors
...
void eval_props() {
    ...
    if (fabs(potEnergy) > 1e6 || fabs(kinEnergy) > 1e6) {
        printf("Process %d: ERROR - Energy too high at step %d\n", sid, stepCount);
        printf("KE=%.6f  PE=%.6f  TE=%.6f\n", kinEnergy, potEnergy, totEnergy);
        MPI_Abort(MPI_COMM_WORLD, 1);
    }

    // Print properties
    if (sid == 0) printf("Step=%d T=%.2f PE=%.2f TE=%.2f\n",
                        stepCount, temperature, potEnergy, totEnergy);
}
    ...
    for (kdd=0; kdd<2; kdd++) {
      ...
      /* Message storing */
        for (i=0; i<nrc; i++) {
              double prev_pos[3];
              for (a=0; a<3; a++) {
                  prev_pos[a] = dbufr[6*i+a];
                  
                  // Validate received positions
                  if (isnan(prev_pos[a]) || isinf(prev_pos[a])) {
                      printf("Process %d: ERROR - Invalid position received\n", sid);
                      MPI_Abort(MPI_COMM_WORLD, 1);
                  }
                  
                  // Enforce periodic boundaries
                  r[n+newim+i][a] = prev_pos[a];
                  while (r[n+newim+i][a] >= al[a]) r[n+newim+i][a] -= al[a];
                  while (r[n+newim+i][a] < 0.0) r[n+newim+i][a] += al[a];
                  
                  // Validate velocity
                  rv[n+newim+i][a] = dbufr[6*i+3+a];
                  if (isnan(rv[n+newim+i][a]) || isinf(rv[n+newim+i][a])) {
                      printf("Process %d: ERROR - Invalid velocity received\n", sid);
                      MPI_Abort(MPI_COMM_WORLD, 1);
                  }
              }
          }
